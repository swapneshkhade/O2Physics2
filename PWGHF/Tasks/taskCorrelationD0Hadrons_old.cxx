...
/// \file taskCorrelationD0Hadrons_old.cxx
...
const TString stringDHadron = "D0,Hadron candidates ";
...
/// D0-Hadron correlation pair filling task, from pair tables - for real data and data-like analysis (i.e. reco-level w/o matching request via MC truth)
struct HfTaskCorrelationD0HadronsOld {
  ...
  void processData(aod::D0HadronPairFull const& pairEntries)
  {
    for (auto& pairEntry : pairEntries) {
     ...
     // }
      ...
      if (massD > signalRegionInner->at(pTBinD) && massD < signalRegionOuter->at(pTBinD)) {
        //in signal region
        std::cout<<deltaPhi<<" "<<deltaEta<<" "<<ptD<<" "<<ptHadron<<" "<<efficiencyWeight<<std::endl;
  ...
  PROCESS_SWITCH(HfTaskCorrelationD0HadronsOld, processData, "Process data", false);
...
/// D-Hadron correlation pair filling task, from pair tables - for MC reco-level analysis (candidates matched to true signal only, but also bkg sources are studied)
  ...
  PROCESS_SWITCH(HfTaskCorrelationD0HadronsOld, processMcRec, "Process MC Reco mode", true);
...
/// D-Hadron correlation pair filling task, from pair tables - for MC gen-level analysis (no filter/selection, only true signal) - Ok for both USL and LS analyses
  void processMcGen(aod::D0HadronPair const& pairEntries)
  {
    for (auto& pairEntry : pairEntries) {
      //define variables for widely used quantities
      double deltaPhi = pairEntry.deltaPhi();
   ...
   PROCESS_SWITCH(HfTaskCorrelationD0HadronsOld, processMcGen, "Process MC Gen mode", false);
...
{
  return WorkflowSpec{adaptAnalysisTask<HfTaskCorrelationD0HadronsOld>(cfgc)};
}